...
| No `.git` repository currently exists in this working copy | Critical | Must be resolved before Phase 0 starts -- a multi-month, multi-phase build with zero version control and no rollback path is not workable |
...
1. **`leads` extended vs. new `customers` table** (Section 8) -- recommendation is extend `leads`, but this is the user's call and the highest-leverage decision in this whole plan.
2. **Git repository status** -- must be resolved first; a multi-phase build cannot safely proceed with no version control.
