...
async function gatherDailyMetrics() {
  ...
  const [
    ...
    supabaseAdmin.from('leads').select('id, name, phone, event_type, source, ai_score, budget').gte('created_at', todayStr).order('created_at', { ascending: false }),
    supabaseAdmin.from('leads').select('id, name, budget').eq('status', 'confirmed').gte('updated_at', todayStr),
    supabaseAdmin.from('proposals').select('id, client_name, total_price, package_name').gte('sent_at', todayStr),
    supabaseAdmin.from('leads').select('status').in('status', ['new_inquiry', 'followup_pending', 'proposal_sent', 'negotiation']),
    supabaseAdmin.from('leads').select('id, name, phone, ai_score, ai_score_reason, budget, event_type').gte('ai_score', 7).not('status', 'in', '("confirmed","rejected")').order('ai_score', { ascending: false }).limit(5),
    supabaseAdmin.from('leads').select('id, name, phone, last_contacted_at, created_at').in('status', ['new_inquiry', 'followup_pending']).not('phone', 'is', null).or(`last_contacted_at.is.null,last_contacted_at.lt.${new Date(Date.now() - 48 * 3600000).toISOString()}`).limit(10),
    supabaseAdmin.from('conversations').select('id, channel').gte('created_at', todayStr),
  ])
  ...
  return {
    ...
    vipLeads: (vipLeads || []).map(l => ({ name: l.name || 'Unknown', phone: l.phone || '', score: l.ai_score || 0, reason: l.ai_score_reason || '', budget: l.budget, event_type: l.event_type })),
...
- Proposals Sent Today: ${metrics.proposalsSent.length}
  ${metrics.proposalsSent.map((p: any) => `  • ${p.client_name} — ${p.package_name} (₹${p.total_price?.toLocaleString('en-IN')})`).join('\n')}
...
Keep it under 250 words. Use emojis naturally. Write in English with Indian warmth.`
  ...
  const summaryText = response.content[0].type === 'text' ? response.content[0].text : 'Good morning! Here is your daily summary.'
...
export async function sendDailySummaryWhatsApp(summary: DailySummaryData): Promise<boolean> {
  const supabaseAdmin = getSupabaseAdmin()
  const { data: setting } = await supabaseAdmin.from('notification_settings').select('value').eq('key', 'daily_summary_whatsapp').single()
  ...
  if (sent) await supabaseAdmin.from('ai_summaries').update({ sent_via_whatsapp: true }).eq('date', summary.date)
  return sent
}

export async function detectAndFlagVIPLeads(): Promise<number> {
  const supabaseAdmin = getSupabaseAdmin()
  const { data: settings } = await supabaseAdmin.from('notification_settings').select('key, value').in('key', ['vip_threshold_score', 'vip_threshold_budget'])
  const thresholdScore = parseInt(settings?.find(s => s.key === 'vip_threshold_score')?.value || '8')

  const { data: candidates } = await supabaseAdmin.from('leads').select('id, name, ai_score, budget, event_type').eq('is_vip', false).gte('ai_score', thresholdScore)
  ...
  for (const lead of candidates) {
    ...
    await supabaseAdmin.from('leads').update({ is_vip: true, vip_reason: reasons.join(', ') }).eq('id', lead.id)
    await supabaseAdmin.from('activity_logs').insert({ lead_id: lead.id, action: 'vip_flagged', description: `Lead flagged as VIP: ${reasons.join(', ')}`, performed_by: 'ai_system' })
    flagged++
  }
