...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    /**
     * Seed one static knowledge source
     */
    if (action === 'seed_one') {
      ...
      if (error) {
        logger.error('knowledge', 'Batch insert failed', error)
    ...
    if (action === 'add_text' && text && source) {
      ...
      if (error) {
        logger.error('knowledge', 'Custom text insert failed', error)
